...
export function FeaturesPage({ initialFeatures }: { initialFeatures?: Feature[] }) {
  const [features, setFeatures] = useState<Feature[]>(initialFeatures ?? [])
  const [searchTerm, setSearchTerm] = useState("")
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    if (initialFeatures === undefined) {
      const load = async () => {
        try {
          const res = await apiService.fetchFeatures()
  ...
  if (error) {
    return (
      <div className="p-8 text-center">
        <p className="text-red-600 mb-4">خطأ في تحميل الميزات: {error}</p>
