...
export function UnitsPage({ initialUnits }: { initialUnits?: Unit[] }) {
  ...
  useEffect(() => {
    if (initialUnits === undefined) {
      const load = async () => {
        try {
          const res = await apiService.fetchUnits()
          setUnits(res.data.data || res.data)
        } catch (err: any) {
          setError(err.response?.data?.message || err.message || "Failed to load units")
        }
      }
      load()
    }
  ...
  if (error) {
    return (
      <div className="p-8 text-center">
        <p className="text-red-600 mb-4">خطأ في تحميل الوحدات: {error}</p>
